...
for i in range(4):
    ...
    train_ann = [obj_ann_ci[m] for m in train_idx] #列表
    val_ann = [obj_ann_ci[j] for j in val_idx]
    train_annotations += train_ann
    val_annotations += val_ann
        
    train_im_idxs = [im_idx_i[m] for m in train_idx] #列表, 需去除重复
    ...
    val_im_idxs = [im_idx_i[n] for n in val_idx]
    # val_im_idx = sorted(set(val_im_idxs),key=val_im_idxs.index)
    val_im_idx = list(set(val_im_idxs))
    val_im_indice += val_im_idx    

train_im_indice = list(set(train_im_indice))
val_im_indice = list(set(val_im_indice))
annotations_info_train = {'images':[annotations_info['images'][i-1] for i in train_im_indice], 
                          'annotations': train_annotations, 
                          'categories': annotations_info['categories']}
annotations_info_val =   {'images':[annotations_info['images'][i-1] for i in val_im_indice], 
                          'annotations': val_annotations, 
                          'categories': annotations_info['categories']}


with  open(os.path.join(root_dir, 'data/underwater_od_data/annotations_json/annotations_trainval.json'), 'w')  as f:
    json.dump(annotations_info, f, indent=4)
with  open(os.path.join(root_dir, 'data/underwater_od_data/annotations_json/annotations_train.json'), 'w')  as f:
    json.dump(annotations_info_train, f, indent=4)
with  open(os.path.join(root_dir, 'data/underwater_od_data/annotations_json/annotations_val.json'), 'w')  as f:
